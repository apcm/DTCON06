...
public class PhaseService {
	...
	public void delete(final Phase phase) {
		...
		phases.remove(phase);
		logHandyWorker.setPlannedPhases(phases);
	...
	public Phase saveForTest(final Phase p) {
		final Phase res = this.phaseRepository.save(p);
		final HandyWorker hw = this.handyWorkerService.findByPrincipal();
		final Collection<Phase> ps = hw.getPlannedPhases();
		ps.add(res);
		hw.setPlannedPhases(ps);
		return res;
	}

	public Collection<Phase> findByHandyWorkerId(final Integer id) {
		return this.phaseRepository.findByHandyWorkerId(id);
	}
}
