...
public class ComplaintService {
	...
	public Complaint saveByCustomer(final Complaint complaint) {
		...
		if (complaint.getId() == 0)
			res = this.saveForTest(complaint);
		else {
			//Logged user must be a customer
			final Authority a = new Authority();
			final UserAccount user = LoginService.getPrincipal();
	...
	public void delete(final Complaint comp) {
		this.complaintRepository.delete(comp);
	}

	public Complaint findOne(final Complaint c) {
		return this.complaintRepository.findOne(c.getId());
	}
}
